    ...
    def getPossibleLetters(self,guessed):
        ...
        for char in LETTERS:
            if char not in guessed:
                if self.prizeMoney<250 and char in VOWELS:
                    continue
                remlist.append(char)
        return remlist

    def getMove(self, category, obscuredPhrase, guessed):
        remlist=self.getPossibleLetters(guessed)
        movelist=self.SORTED_FREQUENCIES
        if len(remlist)==0:
            return 'pass'
        else:
            for char in self.SORTED_FREQUENCIES:
                if char not in remlist:
                    movelist=movelist.replace(char,'')
...
#p=WOFComputerPlayer("Billy",1)
#p.getMove('player','phrase',['R','T','O','X','J','C','K','Z','A','N'])
#p.getPossibleLetters(['R','T','O','X','J','C','K','Z','A','N'])
